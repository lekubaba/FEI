$(document).ready(function(){
    ...
    $('.content-list-aa').click(function(e){
        ...
        var diyaedu = $('#content-list-ee').val();
        var data = {username:username,number:number,zonghefen:zonghefen,jikexishu:jikexishu,chushiedu:chushiedu,diyaedu:diyaedu}; 

        if(creg.test(username)&&myreg.test(number)&&newreg.test(zonghefen)&&newreg.test(jikexishu)&&newreg.test(chushiedu)&&newreg.test(diyaedu)){
           ...
           if(number.length===0){
            ...
            if(diyaedu.length===0){
                 $("#content-list-ee").val("抵押额度不能为空"); 
                 e.preventDefault();
                 return;                
            }
            if(!newreg.test(diyaedu)){
                 $("#content-list-ee").val("输入格式不对"); 
                 e.preventDefault();
                 return;                
            }
        }
